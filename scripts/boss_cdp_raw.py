#!/usr/bin/env python3
"""
BOSS直聘职位抓取 + 规则分析 — 纯 Python/CDP 实现
...
LOCAL_CDP_PROXIES = {"http": "", "https": "", "all": ""}
...
def get_chrome_executable_candidates():
    """Return supported Chromium browser executables in preference order."""
    ...
    if system == "Darwin":
        ...
        return [
            os.path.join(app_dir, app_name, "Contents", "MacOS", executable)
            for app_name, executable in app_executables
    ...
    if system == "Windows":
        ...
        return candidates or ["chrome.exe"]
...
def get_default_chrome_path():
    candidates = get_chrome_executable_candidates()
...
def resolve_chrome_executable(chrome_path=None):
    """Resolve an explicit or automatically detected Chromium executable."""
    requested_path = chrome_path or os.environ.get("BOSS_CHROME_PATH")
    if requested_path:
        requested_path = os.path.abspath(os.path.expanduser(requested_path))
        if os.path.isfile(requested_path):
            return requested_path
        raise ChromeExecutableNotFoundError(
            f"指定的 Chromium 浏览器不存在：{requested_path}"
        )
    ...
    raise ChromeExecutableNotFoundError(
        "未找到可用的 Chromium 浏览器（支持 Google Chrome、Chromium、Edge、Brave）。\n"
        "   Safari 不支持本项目使用的 Chrome DevTools Protocol。\n"
        "   请先安装 Google Chrome，或用 --chrome-path 指定浏览器可执行文件。"
    )
...
DEFAULT_CDP_DATA_DIR = os.path.expanduser("~/.boss-zhipin-python/chrome-profile")
DEFAULT_RESULT_DIR = os.path.expanduser("~/.boss-zhipin-python/job-result")
...
class CDPConnectionError(RuntimeError):
    """The local Chrome DevTools endpoint is missing or malformed."""


def request_local_cdp(cdp_port=DEFAULT_CDP_PORT, timeout=5):
    """Request the local CDP endpoint without inheriting system proxies."""
    if not require_runtime_dependencies("requests"):
        raise CDPConnectionError("缺少 requests，无法连接 Chrome CDP")
    return requests.get(
        f"http://127.0.0.1:{cdp_port}/json/version",
        timeout=timeout,
        proxies=LOCAL_CDP_PROXIES,
    )


def fetch_cdp_version(cdp_port=DEFAULT_CDP_PORT, timeout=5):
    """Return validated /json/version data with an actionable error message."""
    endpoint = f"127.0.0.1:{cdp_port}"
    try:
        response = request_local_cdp(cdp_port, timeout)
    except Exception as exc:
        raise CDPConnectionError(
            f"无法连接本机 Chrome CDP ({endpoint})。"
            f"请先运行 python3 {__file__} --setup-chrome"
        ) from exc

    status_code = getattr(response, "status_code", None)
    if status_code != 200:
        raise CDPConnectionError(
            f"本机 Chrome CDP ({endpoint}) 返回 HTTP {status_code}；"
            "请确认端口未被其他程序占用"
        )
    try:
        data = response.json()
        ...
        raise CDPConnectionError(
            f"本机 Chrome CDP ({endpoint}) 返回了非 JSON 内容"
            f"（Content-Type: {content_type or '未知'}）。"
            "本地 CDP 请求已强制绕过系统代理；请确认该端口由 Chrome 占用"
        ) from exc
    if not isinstance(data, dict) or not data.get("webSocketDebuggerUrl"):
        raise CDPConnectionError(
            f"本机 Chrome CDP ({endpoint}) 响应缺少 webSocketDebuggerUrl；"
            "请关闭占用该端口的程序后重新启动专用 Chrome"
        )
    return data
...
# ============================================================
CSV_COLUMNS = [
    "job_id", "title", "salary", "salary_source", "experience", "location", "tags", "boss_name",
    "boss_active_status",
...
DETAIL_CSV_COLUMNS = [
    "job_id", "title", "company", "salary", "salary_source", "experience", "location",
...
def scrape_list(keyword, city_input, max_pages, filters, output_path,
    ...
    print(f"=== BOSS直聘抓取 ===")
    page_description = max_pages if max_pages is not None else "全部（直到没有下一页）"
    print(f"关键词: {keyword} | 城市: {city_name} | 页数: {page_description}")
    if filter_desc:
    ...
    try:
        pg = 1
        while max_pages is None or pg <= max_pages:
            page_progress = f"{pg}/全部" if max_pages is None else f"{pg}/{max_pages}"
            print(f"--- [{page_progress} 页, {len(all_jobs)} 条已抓] ---")
            ...
            if not jobs:
                print("  ⚠️ 无数据")
                if reached_end:
                    print("  已到最后一页（hasMore=false），结束该搜索组合\n")
                    break
                if max_pages is None:
                    print("  当前页没有职位数据，结束该搜索组合\n")
                    break
                pg += 1
                continue
            ...
            if max_pages is None or pg < max_pages:
                ...
                print(f"  翻页等待 {wait_seconds:.1f}s...\n")
                if wait_seconds > 0:
...
def build_detail_record(job, extracted):
    ...
    if not experience:
        for tag in str(job.get("tags") or "").split("|"):
            tag = tag.strip()
...
def run_smoke_test(cdp_port=DEFAULT_CDP_PORT):
    ...
    try:
        ...
        return 1
            ...
            json.JSONDecodeError, websocket.WebSocketException, TimeoutError,
            CDPConnectionError) as e:
...
def run_check(cdp_port=DEFAULT_CDP_PORT):
    ...
    if requests is None:
        ...
        try:
            data = fetch_cdp_version(cdp_port, timeout=5)
...
def is_cdp_ready(cdp_port):
    try:
        resp = request_local_cdp(cdp_port, timeout=2)
...
def run_setup_chrome(cdp_port=DEFAULT_CDP_PORT, copy_login_state=False,
                     ...
                     login_timeout=DEFAULT_LOGIN_TIMEOUT, chrome_path=None):
    ...
    try:
        chrome_executable = resolve_chrome_executable(chrome_path)
    except ChromeExecutableNotFoundError as e:
        print(f"\n❌ {e}")
        return 1

    print(f"\n启动 Chrome (CDP 端口: {cdp_port})...")
    print(f"   浏览器: {chrome_executable}")
    cmd = [
        chrome_executable,
    ...
    try:
        ...
        print("   请检查 --chrome-path 指向的文件是否可执行。")
        return 1
...
示例:
  ...
  # 只分析已有数据
  %(prog)s --input ~/.boss-zhipin-python/job-result/boss_jobs_20260609_1200.json --analysis --no-detail
  ...
  %(prog)s --setup-chrome
    ...
    p.add_argument("--pages", type=int, default=3, help="抓取页数（正整数）")
    ...
    p.add_argument("--chrome-path", default=None,
                   help="Chromium 浏览器可执行文件路径（默认自动探测，也可设置 BOSS_CHROME_PATH）")
    ...
    if args.input:
        ...
        except (LoginGateError, CDPConnectionError) as e:
            print(str(e))
            sys.exit(1)
